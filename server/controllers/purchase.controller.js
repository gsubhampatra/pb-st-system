...
export const createPurchase = async (req, res) => {
  ...
  const nextInvoiceNo = lastPurchase
    ? parseInt(lastPurchase.invoiceNo.slice(6), 10) + 1
    : 1;
  const invoiceNo = `${year}${month}${day}${nextInvoiceNo.toString().padStart(4, "0")}`;
  ...
  if (!supplierId || !date || !Array.isArray(items) || items.length === 0) {
    return res
      ...
      .json({
        message:
          "Missing required fields: supplierId, date, and non-empty items array",
      });
  }
  for (const item of items) {
    if (
      ...
      return res
        ...
        .json({
          message:
            "Invalid data in items array. Each item needs itemId, positive quantity, and non-negative unitPrice.",
        });
    }
  }
  ...
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      return res
        .status(400)
        .json({
          message: "Database error creating purchase.",
...
export const getPurchases = async (req, res) => {
  ...
  try {
    ...
    const purchases = await prisma.purchase.findMany({
      ...
      include: {
        supplier: {
          // Include basic supplier info
          select: { name: true },
        },
        _count: {
          // Optionally count items per purchase
          select: { items: true },
        },
      },
      orderBy: {
  ...
  try {
    const purchase = await prisma.purchase.findUnique({
      ...
      include: {
        ...
        items: {
          // Include associated purchase items
          include: {
    ...
    if (!purchase) {
      return res
        .status(404)
        .json({ message: `Purchase with ID ${id} not found` });
    }
...
export const updatePurchase = async (req, res) => {
  ...
  if (typeof paidAmount === "undefined" && !status && !items) {
    return res
      ...
      .json({
        message:
          "No valid fields (paidAmount, status, items) provided for update",
      });
  }
  if (typeof paidAmount !== "undefined" && typeof paidAmount !== "number") {
    return res
      .status(400)
      .json({ message: "Invalid paidAmount, must be a number" });
  }
  ...
  if (items) {
    for (const item of items) {
      if (
        ...
        return res
          .status(400)
          .json({ message: "Invalid item, must contain itemId and quantity" });
      }
    }
  }
  ...
  try {
    ...
    if (items) {
      for (const item of items) {
        const itemToUpdate = await prisma.purchaseItem.findFirst({
          where: { itemId: item.itemId, purchaseId: id },
        });
        if (!itemToUpdate) {
          return res
            .status(404)
            .json({
              message: `Purchase item with ID ${item.itemId} not found`,
            });
        }
    ...
    const updatedPurchase = await prisma.purchase.update({
      ...
      include: {
        // Return the updated purchase with details
    ...
    if (
      ...
      return res
        .status(404)
        .json({ message: `Purchase with ID ${id} not found during update.` });
    }
    res
      .status(500)
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const purchaseToDelete = await tx.purchase.findUnique({
        ...
        include: {
          items: {
            // Need items to know quantities and item IDs
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      if (error.code === "P2003") {
        return res
          ...
          .json({
            message:
              "Cannot delete purchase: It might be referenced in other records (e.g., payments).",
          });
      }
    } else if (error.message.includes("not found")) {
    ...
    if (
      error.message.includes("Cannot delete purchase with associated payments")
    ) {
